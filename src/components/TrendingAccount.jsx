...
const ProductTable = () => {
  ...
  const userLog = data?.data?.result;
            ...
            <table className="w-full sm:ml-20">
              <thead>
                <tr className="bg-[#FAFAFB] text-left border-b border-[#EDF2F7]">
                  ...
                  <th className="p-4 text-[#949494] font-semibold">
                    Amount (₦)
                  </th>
          ...
          <div className="md:hidden space-y-6">
            {userLog?.length > 0 ? (
              userLog.map((product, index) => (
                <div
                  key={index}
                  className="border border-[#EDF2F7] rounded-lg p-4 shadow-sm"
                >
                  ...
                  <div className="text-sm text-[#515151] mb-2">
                    <strong>Amount:</strong> ₦{product.amount.toLocaleString()}
                  </div>
